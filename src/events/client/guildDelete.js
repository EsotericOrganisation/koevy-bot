	...
	async execute(guild, client) {
		...
		try {
			await client.users.fetch(ownerId).send({
				content:
					"The server has been deleted or I have been kicked. Here is the invite to the backup server:\n" +
					invite.url,
			});
		} catch (error) {
		...
		console.log(
			chalk.red(
				"The server has been deleted or I have been removed from the server. \nI have created a backup server and sent you an invite. \nIf you have not received the invite, here is a copy of the url: " +
					chalk.blue(invite.url)
			)
		);
	},
};
