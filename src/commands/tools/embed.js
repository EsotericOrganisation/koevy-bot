...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((option) =>
			option.setName("clear").setDescription("Clear the embed of it's settings.")
		)
		.addSubcommand((option) =>
		...
		.addSubcommand((option) =>
			option
				...
				.addBooleanOption((option) =>
					option.setName("ping-everyone").setDescription("Whether to ping everyone.")
				)
		)
		.addSubcommand((option) =>
			option
		...
		switch (interaction.options.getSubcommand()) {
			...
			case "edit": {
				...
				for (let i = 1; i <= 5; i++) {
					...
					const embedValue = embed[interaction.options.getString(`component-${i}`)];
