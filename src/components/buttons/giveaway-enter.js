...
module.exports = {
	...
	async execute(interaction, client) {
		...
		const giveaway = await GiveawaySchema.findOne({
			id: interaction.message.embeds[0].data.author.name.split(/ +/)[1],
		});
		...
		if (
			!member.roles.cache.has(giveaway.data?.requiredRole) &&
			giveaway.data?.requiredRole
		)
		...
		else if (giveaway.data.entries.includes(interaction.user.id))
			return await interaction.reply({
				content: "You have already entered this giveaway!",
				ephemeral: true,
			});
		//Push twice if user has bonus entries role
		else if (member.roles.cache.has(giveaway.data?.bonusRole))
			giveaway.data.entries.push(interaction.user.id, interaction.user.id);
		//Push once if user has required role, but not bonus entry role
		else giveaway.data.entries.push(interaction.user.id);

		//Edit embed
		const editedEmbed = EmbedBuilder.from(
			interaction.message.embeds[0]
		).data.description.split("\n");
		editedEmbed.splice(
			2,
			1,
			"> Total Entries: " + inlineCode(giveaway.data.entries.length)
		);
