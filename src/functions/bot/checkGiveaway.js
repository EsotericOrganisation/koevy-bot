...
const { bold, userMention, ActionRowBuilder, ButtonBuilder } = require("discord.js");
...
const endGiveaway = async (doc, client) => {
    ...
    await gawMessage.reply({
      content: `Congratulations ${winnersArray.map((winner) => userMention(winner) + ",")} on winning ${bold(doc.item)}! Please open a Ticket within 24hrs to claim your rewards.`
    });

    await gawMessage.edit({
      components: [new ActionRowBuilder().setComponents(ButtonBuilder.from(gawMessage.components[0].components[0]).setDisabled(true))]
    });
